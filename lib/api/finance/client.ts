...
export class FinanceAPIClient {
    ...
    constructor() {
        ...
        if (!this.apiKey || !this.secretKey || !this.baseUrl) {
            console.warn("Finance API credentials are not set in environment variables.");
        }
    }
    ...
    async getBalance(includeProjected = false): Promise<FinanceBalanceResponse> {
        return this.request<FinanceBalanceResponse>("GET", "/api/v1/balance", { includeProjected });
    }
    ...
    async getTransactions(filters: {
        ...
        return this.request<FinanceTransactionsResponse>("GET", "/api/v1/transactions", filters);
    }
    ...
    async getBudgets(year?: number, costCenterId?: string): Promise<FinanceBudgetsResponse> {
        return this.request<FinanceBudgetsResponse>("GET", "/api/v1/budgets", { year, costCenterId });
    }

    async getCostCenters(includeHierarchy = false): Promise<FinanceCostCentersResponse> {
        return this.request<FinanceCostCentersResponse>("GET", "/api/v1/cost-centers", { includeHierarchy });
    }

    async getFinancialSummary(year?: number, startMonth?: string, endMonth?: string): Promise<FinanceSummaryResponse> {
        return this.request<FinanceSummaryResponse>("GET", "/api/v1/financial-summary", { year, startMonth, endMonth });
    }
}
