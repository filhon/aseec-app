...
async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(
        `[Sync] Attempt ${attempt}/${maxRetries} failed, retrying in ${delay}ms...`,
        err,
      );
...
async function fetchAndGroupTransactions() {
  ...
  while (hasNext && currentPage <= MAX_PAGES) {
    try {
      ...
      if (currentPage % 10 === 0) {
        console.log(
          `[Sync] Progress: ${transactions.length} transactions fetched so far (page ${currentPage - 1})...`,
        );
      }
    } catch (err) {
  ...
  console.log(
    `[Sync] Total transactions fetched: ${transactions.length} across ${currentPage - 1} pages`,
  );
  ...
  if (filteredTransactions.length < transactions.length) {
    console.warn(
      `[Sync] Client-side filter removed ${transactions.length - filteredTransactions.length} transactions with non-allowed cost centers`,
    );
  }
  ...
  console.log(
    `[Sync] Filtered out: ${filteredByType} non-payable transactions`,
  );
  console.log(
    `[Sync] Grouped into ${Object.keys(groupedData).length} projects`,
  );
...
export async function fetchAggregatedTransactions(
  page = 1,
  limit = 10,
): Promise<{ data: AggregatedTransaction[]; total: number; error?: string }> {
  try {
    ...
    const importedIds = new Set(
      (existingProjects || []).map((p) => p.financial_project_id),
    );
    const importedTitles = new Set(
      (existingProjects || []).map((p) => p.title.toLowerCase()),
    );

    const newProjects = aggregatedArray.filter(
      (g) =>
    ...
    const isApiError =
      message.includes("Finance API Error") ||
      message.includes("cost centers endpoint failed");
    return {
      data: [],
      total: 0,
      error: isApiError
...
export async function searchFinanceProjects(
  query: string,
): Promise<{ results: FinanceTransaction[]; alreadyLinkedCount: number }> {
  try {
    ...
    const groupedData: Record<
      string,
      FinanceTransaction & { _allIds: string[] }
    > = {};
...
export async function getProjectPaidAmount(
  ...
  try {
    ...
    for (const t of result.data || []) {
      ...
      if (
        tDesc.toLowerCase() === desc.toLowerCase() &&
