    ...
    <div className="space-y-3 pt-4 first:pt-0 border-b border-border/50 pb-4 last:border-0 last:pb-0">
      <h4 className="text-sm font-medium leading-none flex justify-between items-center">
        ...
        {selected.length > 0 && (
          <Badge
            variant="secondary"
            className="h-5 px-1.5 text-[10px] min-w-5 flex justify-center"
          >
            {selected.length}
          </Badge>
        )}
      ...
      {hasMore && (
        ...
        >
          {isExpanded ? (
            ...
            <>
              ver mais ({sortedOptions.length - MAX_VISIBLE}){" "}
        ...
        <div className="space-y-4 pt-4 border-b border-border/50 pb-4">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-muted-foreground">
              ...
              }).format(filters.financialRange[0])}{" "}
              -{" "}
              {new Intl.NumberFormat("pt-BR", {
                style: "currency",
                currency: "BRL",
                maximumFractionDigits: 0,
              }).format(filters.financialRange[1])}
            </span>
          </div>
          <Slider
            ...
            onValueChange={(val) =>
              onFilterChange("financialRange", val as [number, number])
            }
            className="py-2"
          />
        </div>
