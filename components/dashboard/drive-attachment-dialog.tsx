...
export function DriveAttachmentDialog({
  open,
  onOpenChange,
  onAdd,
}: DriveAttachmentDialogProps) {
  ...
  const effectiveType: FileType | null =
    (autoDetectedType ?? (manualType as FileType)) || null;

  // Show the type selector only when we can't auto-detect
  const needsTypeSelection =
    url.length > 10 && isDriveUrl(url) && autoDetectedType === null;
  ...
  const handleAdd = () => {
    ...
    const resolvedType =
      autoDetectedType ?? (manualType as FileType | undefined);
    if (!resolvedType) {
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-2">
          <p className="text-sm text-muted-foreground">
            Compartilhe o arquivo no Google Drive (
            <span className="font-medium">Qualquer pessoa com o link</span>),
            cole o link abaixo.
          </p>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="document">
                    <span className="flex items-center gap-2">
                      <FileText className="h-4 w-4" /> Documento / PDF
                    </span>
                  </SelectItem>
                </SelectContent>
              </Select>
