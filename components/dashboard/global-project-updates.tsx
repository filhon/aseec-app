...
import { ProjectPost } from "@/components/dashboard/data";
import { getGlobalProjectPosts } from "@/lib/services/project-service";
...
export function GlobalProjectUpdates({
  onlyToday = false,
  showViewAll = false,
  variant = "compact",
  filterIds,
}: GlobalProjectUpdatesProps) {
  ...
  useEffect(() => {
    async function fetchPosts() {
      setLoading(true);
      try {
        const posts = await getGlobalProjectPosts();
        setAllPosts(posts as unknown as EnrichedPost[]);
      } catch (error) {
          ...
          <div className="space-y-6 py-4">
            {loading ? (
              ...
              displayedPosts.map((item) => (
                <div
                  key={`${item.projectId}-${item.id}`}
                  className="flex gap-3 relative pb-6 last:pb-0"
                >
                  {/* Timeline line */}
