...
export function NewProjectForm() {
  ...
  const selectFinancialProject = (project: FinanceTransaction) => {
    ...
    if (!form.getValues("institution"))
      form.setValue("institution", project.supplier || "");
            ...
            <div className="flex-1 space-y-4">
              <div>
                <h3 className="text-sm font-medium">
                  Vincular Projeto Financeiro
                </h3>
                <p className="text-sm text-muted-foreground">
                  Pesquise para conectar este projeto aos dados financeiros
                  (orçamento, gastos).
                </p>
              </div>
                      ...
                      {searchResults.results.map((p) => (
                        ...
                        >
                          ...
                          <span className="text-xs text-muted-foreground flex items-center justify-between mt-1 w-full">
                            <span>
                              {p.supplier || "Sem Instituição"}{" "}
                              {p.costCenter?.code
                                ? `• CC: ${p.costCenter.code}`
                                : ""}
                            </span>
                      ...
                      {searchResults.alreadyLinkedCount > 0 && (
                        <div className="px-3 py-2.5 text-xs text-muted-foreground bg-amber-50/50 dark:bg-amber-900/10 border-t flex flex-col shrink-0 mt-auto sticky bottom-0">
                          <span className="font-medium text-amber-700 dark:text-amber-400">
                            {searchResults.alreadyLinkedCount} projeto(s)
                            financeiro(s) não estão na lista
                          </span>
                          <span>
                            Eles já se encontram vinculados a outros projetos da
                            base de dados.
                          </span>
                        </div>
                      )}
                  ...
                  {searchQuery.length >= 2 &&
                    ...
                    searchResults.alreadyLinkedCount === 0 && (
                      <div className="absolute top-full left-0 right-0 mt-1 bg-popover border rounded-md shadow-lg z-10 p-4 text-center text-sm text-muted-foreground">
                        Nenhum projeto financeiro encontrado para &quot;
                        {searchQuery}&quot;.
                      </div>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between bg-background border p-3 rounded-md">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5">
                      ...
                      {selectedFinancialProject.costCenter?.code &&
                        `• CC: ${selectedFinancialProject.costCenter.code}`}
                      •{" "}
                      <span className="font-medium text-foreground">
                        {new Intl.NumberFormat("pt-BR", {
                          style: "currency",
                          currency: "BRL",
                        }).format(selectedFinancialProject.amount)}
                      </span>
                    </span>
                  </div>
                  <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="col-span-1 md:col-span-2 space-y-4">
            <div className="flex flex-col space-y-2">
              ...
              />
              <FormDescription>
                Digite o endereço ou CEP para preencher automaticamente os dados
                de localização.
              </FormDescription>
            </div>
