...
} from "@/lib/actions/finance/sync-actions";
import {
  Card,
  CardContent,
  CardDescription,
...
export function ProjectSyncList() {
  ...
  const fetchUnsynced = async (currentPage = page, currentLimit = limit) => {
    ...
    setError(null);
    try {
      const result = await fetchAggregatedTransactions(
        currentPage,
        currentLimit,
      );
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      for (const proj of projectsToSync) {
        ...
        const rawDate = proj.dates[0]
          ? new Date(proj.dates[0]).toISOString()
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-muted-foreground border border-dashed rounded-lg bg-destructive/5 border-destructive/20">
        ...
        <h3 className="text-lg font-semibold text-destructive">
          Sistema financeiro indisponível
        </h3>
        <p className="text-sm text-center max-w-sm mt-1">{error}</p>
        <Button
          variant="outline"
          className="mt-4"
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-muted-foreground border border-dashed rounded-lg bg-muted/10">
        ...
        <p className="text-sm text-center max-w-sm mt-1">
          Não encontramos novos projetos no sistema financeiro para importar no
          momento.
        </p>
        <Button
          variant="outline"
      ...
      <CardHeader className="border-b">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Encontramos {projects.length} novos projetos no sistema
              financeiro.
            </CardDescription>
          </div>
      ...
      <CardContent className="p-0">
        ...
        <div className="md:hidden">
          ...
          {projects.map((project) => (
            <div
              key={project.id}
              className="p-4 border-b last:border-0 flex flex-col gap-3 active:bg-muted/50 transition-colors"
              onClick={() => {
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 space-y-1.5">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Badge
                      variant="outline"
                      className="bg-blue-50 text-blue-700 border-blue-200 text-[10px] h-5 px-1.5 shrink-0"
                    >
                      Novo
                    </Badge>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    {project.transactionCount > 1 && (
                      <span className="bg-muted px-1.5 py-0.5 rounded">
                        {project.transactionCount} parcelas
                      </span>
                    )}
                    <span className="bg-muted px-1.5 py-0.5 rounded">
                      CC: {project.costCenterCode || "N/A"}
