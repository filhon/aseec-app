      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Gerencie seus projetos, locais e entidades favoritas.
          </p>
        </div>
      </div>
      ...
      {!hasFavorites ? (
        <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed rounded-lg bg-muted/10 h-[50vh]">
          ...
          <p className="text-muted-foreground max-w-md mb-6">
            Você ainda não adicionou nenhum item aos favoritos. Navegue pelos
            projetos, países ou entidades e clique no ícone de coração para
            salvar o que é importante para você.
          </p>
      ...
      <CardContent className="pt-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {visibleItems.map((item) => (
            <div
              key={item.id}
              className="group relative flex items-start gap-3 p-3 rounded-lg border hover:shadow-md transition-all hover:bg-muted/30 bg-card"
            >
              {item.image ? (
              ...
              <div className="flex-1 min-w-0">
                <Link
                  href={getLink(item.id, item.type)}
