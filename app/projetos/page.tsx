...
export default function ProjectsPage() {
  ...
  const [mapFilteredIds, setMapFilteredIds] = useState<Set<string> | null>(
    null,
  );
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Gerencie e acompanhe o progresso de todos os projetos.
          </p>
        </div>
            ...
            >
              <Card className="h-full hover:border-primary/50 transition-all hover:shadow-md cursor-pointer relative">
                <CardHeader className="pb-3 space-y-2">
                  ...
                  <div>
                    <CardTitle
                      className="text-base group-hover:text-primary transition-colors line-clamp-1"
                      title={project.title}
                    >
                      {project.title}
                    </CardTitle>
                      ...
                      {canViewFinancials ? (
                        <div className="flex flex-col">
                          ...
                          <span className="font-bold text-foreground text-sm">
                            ...
                            }).format(Number(project.investment) || 0)}
                          </span>
                        </div>
                      ) : (
                        <div className="flex flex-col">
...
function BadgeStatus({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase border ${style}`}
    >
      {label}
    </span>
  );
