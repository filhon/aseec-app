...
const MOCK_RESULTS: SearchResult[] = [
  {
    ...
    description:
      "Expansão missionária e apoio comunitário na região amazônica.",
    type: "project",
    category: "missionary",
  ...
  },
  {
    ...
    description:
      "Projeto humanitário para levar água potável a comunidades carentes.",
    type: "project",
  ...
  },
  {
    ...
    description:
      "Assistência social e integração para famílias refugiadas em zonas de conflito.",
    type: "project",
    category: "social",
    financialValue: 8000,
    status: "concluido",
...
const indicationLabels: Record<string, string> = {
  ind_1: "Pr. Marcos Silva",
  ind_2: "Dra. Helena Costa",
  ind_3: "Dir. Roberto Almeida",
  ind_4: "Pra. Julia Ferreira",
};
...
const tagLabels: Record<string, string> = {
  tag_guerra: "Guerra",
  tag_construcao: "Construção",
  tag_fome: "Combate à Fome",
  tag_criancas: "Crianças em Risco",
  tag_agua: "Água Potável",
  tag_refugiados: "Refugiados",
  tag_saude_mental: "Saúde Mental",
};
...
function SearchContent() {
  ...
  const filteredResults = useMemo(() => {
    return MOCK_RESULTS.filter((item) => {
      ...
      if (item.financialValue !== undefined) {
        if (
          item.financialValue < filters.financialRange[0] ||
          item.financialValue > filters.financialRange[1]
        )
          return false;
      }
      if (
        filters.entities.length > 0 &&
        (!item.entityId || !filters.entities.includes(item.entityId))
      )
        return false;
      if (
        filters.responsible.length > 0 &&
        (!item.responsibleId ||
          !filters.responsible.includes(item.responsibleId))
      )
        return false;
      if (
        filters.indication.length > 0 &&
        (!item.indicationId || !filters.indication.includes(item.indicationId))
      )
        return false;
      if (
      ...
      if (
        ...
        (!item.extensionId || !filters.extension.includes(item.extensionId))
      )
        return false;
      if (filters.tags.length > 0) {
        if (!item.tagIds) return false;
        if (!filters.tags.some((tag) => item.tagIds?.includes(tag)))
          return false;
      }
  ...
  const counts = useMemo(() => {
    const baseSet = MOCK_RESULTS.filter(
      (item) =>
        item.title.toLowerCase().includes(initialQuery.toLowerCase()) ||
        item.description?.toLowerCase().includes(initialQuery.toLowerCase()),
    );
    ...
    const inc = (cat: string, id?: string) => {
      if (id) newCounts[cat][id] = (newCounts[cat][id] || 0) + 1;
    };
    ...
    baseSet.forEach((item) => {
      ...
      if (item.thanked !== undefined)
        inc("thanked", item.thanked ? "yes" : "no");
  ...
  const handleFilterChange = (
    category: keyof AdvancedFiltersState,
    ...
    if (category === "financialRange") {
      setFilters((prev) => ({
        ...prev,
        financialRange: value as [number, number],
      }));
    } else {
      setFilters((prev) => {
  ...
  const activeFiltersList = useMemo(() => {
    ...
    filters.entities.forEach((v) =>
      list.push({
        label: entityLabels[v] || v,
        category: "entities",
        value: v,
      }),
    );
    filters.responsible.forEach((v) =>
      list.push({
        label: responsibleLabels[v] || v,
        category: "responsible",
        value: v,
      }),
    );
    filters.indication.forEach((v) =>
      list.push({
        label: indicationLabels[v] || v,
        category: "indication",
        value: v,
      }),
    );
    filters.status.forEach((v) =>
      list.push({ label: statusLabels[v] || v, category: "status", value: v }),
    );
    filters.extension.forEach((v) =>
      list.push({
        label: extensionLabels[v] || v,
        category: "extension",
        value: v,
      }),
    );
    filters.tags.forEach((v) =>
      list.push({ label: tagLabels[v] || v, category: "tags", value: v }),
    );
    filters.thanked.forEach((v) =>
      list.push({
        label: thankedLabels[v] || v,
        category: "thanked",
        value: v,
      }),
    );
  ...
  const isFinancialChanged =
    filters.financialRange[0] > 0 || filters.financialRange[1] < 100000;
            ...
            {(activeFiltersList.length > 0 || isFinancialChanged) && (
              <div className="flex flex-wrap gap-2 items-center pt-2">
                {isFinancialChanged && (
                  <Badge
                    variant="secondary"
                    className="px-2 py-1 gap-1 hover:bg-muted"
                  >
                    $$: {filters.financialRange[0]} -{" "}
                    {filters.financialRange[1]}
                    <X
            ...
            {filteredResults.length === 0 ? (
              ...
              filteredResults.map((result) => {
                ...
                return (
                  <Card
                    key={result.id}
                    className="hover:bg-muted/50 transition-colors cursor-pointer group py-0"
                  >
                    <CardHeader className="p-4 pb-2">
                      <div className="flex justify-between items-start">
                        <div className="flex gap-2 items-center flex-wrap">
                          <Badge
                            variant="outline"
                            className="capitalize flex gap-1 items-center font-normal"
                          >
