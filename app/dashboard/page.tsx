...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = await getProjectsForDashboard();
        setProjects(data);
      } catch (error) {
    ...
    (
      ...
      return projects.filter((project) => {
        ...
        const matchesExtension =
          exclude === "extension" ||
          extensionFilter === "all" ||
          project.extension === extensionFilter;
        const matchesYear =
          exclude === "year" ||
          yearFilter === "all" ||
          project.investmentByYear.some(
            (i) => i.year.toString() === yearFilter,
          );
  ...
  const stats = useMemo(() => {
    ...
    filteredProjects.forEach((p) => {
      // 1. Total Invested
      totalInvested += Number(p.investment) || 0;

      // 2. Reached People (for Cost per Beneficiary)
      if (p.reached_people) totalReachedPeople += p.reached_people;

      // 3. Approved Value (for Execution Rate)
      totalApproved += Number(p.approved_value) || Number(p.investment) || 0;

      // 4. Delayed Projects
      if (p.status === "em_andamento" && p.end_date && p.end_date < today) {
        delayedProjects++;
      }
    ...
    const costPerBeneficiary =
      totalReachedPeople > 0 ? totalInvested / totalReachedPeople : 0;
    const executionRate =
      totalApproved > 0 ? Math.round((totalInvested / totalApproved) * 100) : 0;

    let yoyGrowth = 0;
    if (investedLastYear > 0) {
      yoyGrowth = Math.round(
        ((investedCurrentYear - investedLastYear) / investedLastYear) * 100,
      );
    }
  ...
  const insights = useMemo(() => {
    ...
    filteredProjects.forEach((p) => {
      ...
      countryInvestment[country] =
        (countryInvestment[country] || 0) + (Number(p.investment) || 0);
    });
    ...
    filteredProjects.forEach((p) => {
      institutionInvestment[p.institution] =
        (institutionInvestment[p.institution] || 0) +
  ...
  const availableYears = useMemo(() => {
    ...
    return Array.from(
      new Set(projects.flatMap((p) => p.investmentByYear.map((i) => i.year))),
    ).sort((a, b) => b - a);
  }, [getFilteredProjects]);
  ...
  const activeFilters = useMemo(() => {
    ...
    if (yearFilter !== "all")
      filters.push({ id: "year", label: `Ano: ${yearFilter}`, type: "year" });
    ...
    if (tagFilter !== "all")
      filters.push({ id: "tag", label: `Tag: ${tagFilter}`, type: "tag" });
    if (statusFilter !== "all")
      filters.push({
        id: "status",
    ...
    if (extensionFilter !== "all")
      filters.push({
        id: "extension",
        label: `Extensão: ${extensionFilter}`,
        type: "extension",
      });
    return filters;
  }, [
  ...
  return (
    <ProtectedRoute
      ...
      accessDeniedMessage="Você não tem permissão para acessar o dashboard."
    >
      <div className="space-y-6 py-4 lg:py-8 animate-in fade-in duration-500">
              ...
              <Sheet>
                ...
                <SheetContent side="right">
                  <SheetHeader>
                    ...
                    <SheetDescription>
                      Refine a visualização dos projetos.
                    </SheetDescription>
                  </SheetHeader>
          ...
          {activeFilters.length > 0 && (
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {activeFilters.map((filter) => (
                <div
                  key={filter.id}
                  className="flex items-center gap-1.5 bg-secondary text-secondary-foreground px-3 py-1 rounded-full text-xs font-medium"
                >
                  {filter.label}
                  <button
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Visão consolidada e estratégica dos investimentos.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <Card
            className={`col-span-1 shadow-sm hover:shadow-md transition-shadow ${!isExpanded ? "hidden md:block" : ""}`}
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {stats.costPerBeneficiary > 0
                  ? formatCurrency(stats.costPerBeneficiary)
                  : "N/A"}
              </div>
          ...
          >
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${stats.yoyGrowth >= 0 ? "text-emerald-600" : "text-red-600"}`}
              >
                {stats.yoyGrowth >= 0 ? "+" : ""}
                {stats.yoyGrowth}%
              </div>
          ...
          {/* 5. Schedule Risk */}
          <Card
            className={`col-span-1 shadow-sm hover:shadow-md transition-shadow ${stats.delayedProjects > 0 ? "border-l-4 border-l-red-500 bg-red-50/20 dark:bg-red-900/10" : ""} ${!isExpanded ? "hidden md:block" : ""}`}
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                Risco de Prazo
              ...
              <AlertTriangle
                className={`h-4 w-4 ${stats.delayedProjects > 0 ? "text-red-500" : "text-muted-foreground"}`}
              />
            </CardHeader>
            <CardContent>
              <div
                className={`text-2xl font-bold ${stats.delayedProjects > 0 ? "text-red-600 dark:text-red-400" : ""}`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card className="lg:col-span-2 shadow-sm border-0 bg-gradient-to-br from-card to-muted/20">
            <CardHeader>
              ...
              <CardDescription>
                Evolução do investimento ao longo dos anos para os projetos
                selecionados.
              </CardDescription>
            </CardHeader>
          ...
          <div className="space-y-6">
            ...
            <Card className="shadow-sm">
              <CardHeader className="pb-3">
                <CardTitle className="text-base font-semibold flex items-center justify-between">
                  ...
                  <Link
                    ...
                    className="text-xs font-normal text-muted-foreground hover:underline cursor-pointer text-primary"
                  >
                    Ver todos
                  </Link>
                </CardTitle>
              ...
              <CardContent className="space-y-0.5">
                {insights.topCountries.length > 0 ? (
                  insights.topCountries.map(([country, value], index) => {
                    ...
                    return (
                      <Link
                        href={`/dashboard/paises/${slug}`}
                        key={country}
                        className="flex items-center justify-between text-sm group hover:bg-muted/50 p-2 rounded-md -mx-2 transition-colors cursor-pointer"
                      >
                        <div className="flex items-center gap-2">
                          <span
                            className={`flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold ${index < 3 ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                          >
                            {index + 1}
                          </span>
                          {code ? (
            ...
            <Card className="shadow-sm">
              ...
              <CardContent className="space-y-0.5">
                {insights.topInstitutions.length > 0 ? (
                  insights.topInstitutions.map(([inst, value]) => {
                    const slug = slugify(inst);
                    return (
                      <Link
                        href={`/dashboard/entidades/${slug}`}
                        key={inst}
                        className="flex items-center justify-between text-sm group hover:bg-muted/50 p-2 rounded-md -mx-2 transition-colors cursor-pointer"
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className="font-medium truncate max-w-[120px] group-hover:text-primary transition-colors"
                            title={inst}
                          >
                            {inst}
                          </span>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-bold tracking-tight hidden lg:block">
            Projetos Listados ({filteredProjects.length})
          </h2>
                    ...
                    <div className="space-y-2 text-sm text-muted-foreground">
                      ...
                      <div className="pt-2 border-t mt-3 flex justify-between items-center">
                        <span className="font-semibold text-foreground">
                          {formatCurrency(Number(project.investment) || 0)}
                        </span>
