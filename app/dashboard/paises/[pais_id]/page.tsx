...
export default function CountryPage({
  ...
  const stats = useMemo(() => {
    ...
    filteredProjects.forEach(
      (p) =>
        (instMap[p.institution] = (instMap[p.institution] || 0) + p.investment),
    );
    const topInstitution = Object.entries(instMap).sort(
      ([, a], [, b]) => b - a,
    )[0];
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center h-[50vh]">
        ...
        <p className="text-muted-foreground mt-2">
          Não encontramos nenhum registro para este país.
        </p>
        <Button
          variant="outline"
          className="mt-4"
    ...
    <div className="space-y-8 py-8 animate-in fade-in duration-500 container mx-auto">
                  ...
                  <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-2">
                    ...
                    {code && (
                      ...
                      <img
                        ...
                        width={24}
                        alt={countryName}
                        className="rounded-sm shadow-sm md:hidden"
                      />
                    )}
                  </h1>
                  <FavoriteButton
                    ...
                    image={
                      code ? `https://flagcdn.com/w160/${code}.png` : undefined
                    }
                    className="h-8 w-8"
                  />
                </div>
                <p className="text-muted-foreground mt-1">
                  Visão detalhada dos investimentos e projetos.
                </p>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <CardContent>
            <div
              ...
              title={stats.topInstitution?.[0] || "-"}
            >
              {stats.topInstitution?.[0] || "-"}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              {stats.topInstitution
                ? formatCurrency(stats.topInstitution[1])
                : "R$ 0,00"}
            </p>
          </CardContent>
        </Card>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-bold tracking-tight">
          Projetos em {countryName} ({filteredProjects.length})
        </h2>
